...
description: Coordinate implementation and review with native sub-agents in one chat, or authorized parallel child chats with inherited task context under a headquarters (HQ) coordinator. Use task-fit models, keep supervision, integration and final validation with HQ, and retain optional read-only external advisers. Use for complex implementation, independent parallel workstreams, cross-module changes, architecture review, high-risk fixes, PR review, CI triage, or multi-agent coordination.
---
...
An explicit instruction such as "create child chats when parallel work is useful" lets HQ choose the split for that task without asking again. Compare useful concurrent work with context-transfer and integration overhead before dispatch. Child chats are subordinate through their assignment and HQ's tracking, not an app-enforced parent/child permission boundary. Prefer a supported conversation fork when relevant completed history should carry over; always hand off the current task, required skills and data explicitly. Do not assume live synchronization or that a fork includes the active turn.
...
- For authorized separate chats, discover the app's `fork_thread`, `list_projects`, `create_thread`, `list_threads`, `wait_threads`, `read_thread`, and `send_message_to_thread` tools. Their schemas govern history inheritance, chat creation, model selection, and lifecycle; a model listed there does not establish native `spawn_agent` availability. Do not substitute chat creation or an external CLI for a rejected native model.
...
| Native child model | Default reasoning effort | Use when |
| --- | --- | --- |
| `gpt-6-astra` | `xhigh` (Extra High) | The hardest bounded reasoning, architecture, debugging, or independent high-risk review needs the strongest available model. |
| `gpt-5.6-sol` | `high` | Complex implementation or review needs strong reasoning across multiple files. |
| `gpt-5.6-terra` | Runtime default | Routine implementation, fixes, or review needs balanced capability and cost. |
| `gpt-5.6-luna` | `high` | A well-specified edit, focused search, or check has a narrow scope. |
...
Use the table's reasoning defaults for native children and for manager chats whose selected model is known and whose tool permits the setting. Pass `reasoning_effort` to native spawns and `thinking` to app task calls. Do not maximize effort automatically. A supported user-selected effort takes precedence; never pass a level that the selected model does not advertise. If an explicit choice is unsupported, report the limitation without silently lowering it. If a table default is unsupported, report that fact and use the model's advertised runtime default when available, otherwise leave that assignment pending while independent work continues. For Terra, Spark, or other models without a table effort, use their advertised default; omit the effort only when the tool safely resolves a compatible default. Do not guess an unknown manager model or override the running HQ to apply this table.
