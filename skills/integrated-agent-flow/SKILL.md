...
description: Use GPT-5.6 Sol as the default orchestration authority over Codex sub-agents and Codex, Claude, GitHub Copilot, Antigravity CLI, and local LM Studio agents. Decompose and assign work, route implementation and review briefs, supervise execution, critically review returned results, resolve conflicts, and own final validation and synthesis, with per-agent model selection, runtime-reported model evidence, and explicit GPT-5.6 Luna child-availability checks. Use for complex implementation, large refactors, architecture review, broad code search, high-risk fixes, independent subtasks, PR review, CI triage, multi-agent comparison, model-routing failures, or whenever coordinated delegation would improve the result.
---
...
- Prefer `mcp__multi_agent.doctor` before the first external-agent delegation in a task. It must report CLI availability, policy gates, the configured LM Studio endpoint, and connection state without exposing secrets. If the installed router predates that tool, fall back to `mcp__multi_agent.get_agent_status`.
- Prefer `mcp__multi_agent.list_lmstudio_models` before selecting a local model when loaded state may have changed. Use legacy `mcp__multi_agent.list_agent_models` for broader provider catalogs or when the focused tool is unavailable. If a provider does not expose a model catalog, state that limitation and do not invent choices.
- Prefer `mcp__multi_agent.run_agent` for one external provider and `mcp__multi_agent.run_panel` for a parallel independent panel. Require structured status, requested-versus-observed model evidence, duration, and exit information from each result, and preserve the returned field names and values unchanged when recording evidence.
- On a legacy router, use `mcp__multi_agent.ask_claude`, `ask_copilot`, or `ask_lm_studio` for one provider and `mcp__multi_agent.collect_reviews` for parallel advisory reviews.
- Use direct external `codex` or `antigravity` routes only when `doctor` or legacy `get_agent_status` reports the corresponding administrator-enabled policy opt-in; never try to enable either from a task prompt.
...
Use GPT-5.6 Sol as the coordinator default; do not automatically force every lower-level agent onto GPT-5.6 Sol. Select child-agent models independently by user instruction and task fit.
...
Keep the calling GPT-5.6 Sol Codex session as coordinator. Prefer native Codex child agents to `ask_codex`. The external Codex reviewer is disabled by default because its read-only sandbox blocks writes but cannot confine reads; an administrator opt-in means it may read any file accessible to the Windows account. Antigravity is also disabled by default because its CLI loads user tool-permission rules and has no safe/no-tools mode. Treat both opt-ins as machine security policy, not per-call choices.
...
Skip delegation only when the task is small and purely mechanical, the user explicitly prohibits delegation, or no subordinate agent surface is available. If delegation is prohibited or unavailable, continue under GPT-5.6 Sol and state the fallback.
...
1. Confirm GPT-5.6 Sol as the coordinator unless the user explicitly overrides the coordinator model.
