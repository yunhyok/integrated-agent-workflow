...
3. For Git work, prefer separate worktrees and record the actual base commit. The app defaults to the project default branch. Supply `startingState` only when the user explicitly requested a particular Git state; do not invent `branchName`, a new branch, or a current-working-tree request. `working-tree` means the selected project's checkout on its configured host; it does not transfer HQ's dirty files to another host or apply to a non-Git `local` target. If needed inputs are uncommitted or absent from that default base, keep the affected work pending or local until an authorized transfer/base choice is settled. Have the manager verify its actual checkout/base before edits. For non-Git or explicitly shared local checkouts, require disjoint paths and a single owner of Git/index/build-output mutations; serialize overlapping writes. A separate chat by itself does not isolate files.
...
5. Before the first creation attempt, record a unique workstream marker in HQ's table and include it in the manager brief. This is a correlation marker, not an API idempotency guarantee. Record the creation response immediately. A ready `threadId` and `hostId` identify a usable task. A queued `clientThreadId` is a setup handle, not a ready task ID: do not send it to `wait_threads`, `read_thread`, or `send_message_to_thread`. Resolve the ready ID using the app's documented completion result or `list_threads` evidence, matching project/title and creation context; use `read_thread` to confirm the brief's marker when necessary, and do not guess if ambiguous. Preserve returned titles verbatim when naming tasks.
